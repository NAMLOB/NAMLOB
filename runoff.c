...
void tabulate(void)
{
    for (int v = 0; v < voter_count; v++)
    {
        ...
        {
            if (!candidates[preferences[v][p]].eliminated)
            {
                candidates[preferences[v][p]].votes++;
                break;
            }
        }
    }
...
bool print_winner(void)
{
    ...
    {
        if ((float) candidates[w].votes / (float) voter_count > 0.5)
        {
            printf("%s\n", candidates[w].name);
            return true;
        }
...
int find_min(void)
{
    ...
    {
        if (!candidates[l].eliminated && lowest.votes > candidates[l].votes)
        {
            lowest.votes = candidates[l].votes;
        }
    }
    return lowest.votes;
}
